...
const Sidebar = ({ isOpen, closeSidebar }) => {
  ...
  const fetchActivities = useCallback(() => {
    if (activeProjectId && activeProjectId !== 'login' && activeProjectId !== 'signup') {
      axios.get(`http://localhost:9000/project/${activeProjectId}/activity`)
        .then((res) => {
    ...
    <>
      ...
      `}>
        ...
        <div className="flex-1 py-6 overflow-y-auto px-4">
          ...
          <div className="mb-6">
            <h4 className="px-2 text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Main</h4>
            <Link 
              to="/" 
              onClick={closeSidebar}
          ...
          {activeProjectId && activeProjectId !== 'login' && activeProjectId !== 'signup' && (
            <div className="mt-6 pt-5 border-t border-slate-200">
              ...
              <div className="bg-slate-50 border border-slate-100 rounded-xl p-3 max-h-[250px] overflow-y-auto shadow-xs">
                {activities.length === 0 ? (
                  <p className="text-[11px] text-slate-400 italic text-center py-2">No activities yet.</p>
                ) : (
