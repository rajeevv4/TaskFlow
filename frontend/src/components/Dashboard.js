...
const Dashboard = () => {
    ...
    const handleDeleteProject = async (e, projectId) => {
        ...
        if (window.confirm("Are you sure you want to delete this project? All associated tasks will be lost.")) {
            try {
                const res = await axios.delete(`http://localhost:9000/project/${projectId}`)
    ...
    const handleExportReport = async (e, projectId, projectTitle) => {
        ...
        try {
            ...
            link.href = url
            link.setAttribute('download', `TaskFlow-Report-${projectTitle.replace(/[^a-zA-Z0-9]/g, '_')}.csv`)
        ...
        <div className="w-full px-6 py-8 overflow-y-auto h-[calc(100vh-56px)] bg-slate-50/50">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">Manage and track your projects, tasks, and productivity.</p>
                </div>
                <button
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center space-x-4">
                    <div className="p-3 bg-indigo-50 rounded-lg text-indigo-600">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 12.75V12A9 9 0 0112 3v8.25m0 0h7.5A2.25 2.25 0 0118 13.5h-5.25v5.25a2.25 2.25 0 01-2.25-2.25v-3.75z" />
                        </svg>
                    </div>
                    <div>
                ...
                {projects.length === 0 ? (
                    <div className="bg-white border border-slate-200 rounded-xl p-12 text-center shadow-sm">
                        <div className="w-20 h-20 bg-slate-50 text-slate-400 rounded-full flex items-center justify-center mx-auto mb-4">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-10 h-10">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M6 6.878V6a2.25 2.25 0 012.25-2.25h7.5A2.25 2.25 0 0118 6v.878m-12 0c.235-.083.487-.128.75-.128h10.5c.263 0 .515.045.75.128m-12 0A2.25 2.25 0 004.5 9v.878m13.5-3c.235-.083.487-.128.75-.128H19.5A2.25 2.25 0 0121.75 9v11.25a2.25 2.25 0 01-2.25 2.25H4.5a2.25 2.25 0 01-2.25-2.25V9a2.25 2.25 0 012.25-2.25h.75" />
                            </svg>
                        </div>
                        <h3 className="text-lg font-semibold text-slate-700 mb-1">No projects found</h3>
                        <p className="text-slate-500 max-w-sm mx-auto mb-6">Get started by creating your first project to organize your workflows.</p>
                        <button
                            onClick={() => setCreateModalOpen(true)}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {projects.map((project) => {
                            ...
                            const projCompleted = tasks.filter(t => t.stage === 'Done').length
                            const projPending = projTotal - projCompleted
                            const progressPercent = projTotal > 0 ? Math.round((projCompleted / projTotal) * 100) : 0
                                ...
                                >
                                    <div className="flex justify-between items-start mb-3">
                                        ...
                                        <div className="flex items-center space-x-1 absolute right-6 top-6">
                                            <button
                                                onClick={(e) => handleExportReport(e, project._id, project.title)}
                                    ...
                                    <div className="grid grid-cols-3 gap-2 py-2.5 px-3 bg-slate-50 rounded-lg text-xs text-center border border-slate-100 mb-4">
                                        <div>
                                            <span className="block text-slate-400 font-medium mb-0.5">Tasks</span>
                                            <span className="font-semibold text-slate-700">{projTotal}</span>
                                        </div>
                                        <div>
                                            <span className="block text-slate-400 font-medium mb-0.5">Completed</span>
                                            <span className="font-semibold text-emerald-600">{projCompleted}</span>
                                        </div>
                                        <div>
                                            <span className="block text-slate-400 font-medium mb-0.5">Pending</span>
                                            <span className="font-semibold text-amber-600">{projPending}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center space-x-2 text-xs">
                                        ...
                                        {projTotal === 0 && (
                                            <span className="text-slate-400 italic">No tasks added yet</span>
                                        )}
                                    </div>
                                </Link>
                            )
            ...
            {/* Modals */}
            <AddProjectModal isModalOpen={isCreateModalOpen} closeModal={() => setCreateModalOpen(false)} />
            {isEditModalOpen && (
                <AddProjectModal
